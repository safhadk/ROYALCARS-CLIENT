...
function Chat() {
    ...
    let name = useSelector((state) => state.Client.name);
    ...
    return (
      <div className="App">
        {!showChat ? (
          ...
          <ChatsComponent socket={socket} username={username} room={room} ownerId={location.state?location.state.ownerId:"sample id" }  />
        )}
      </div>
    );
  }
